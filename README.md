...
- **Click to navigate** — clicking an annotation row scrolls the reader to it (toggleable); double-click always navigates
...
- **Templated export** — Mustache-subset templates (`{{var}}`, `{{#section}}`, `{{^inverted}}`, self-referencing partials for subfolder traversal) with Markdown and HTML defaults, an include-subfolders toggle, ordering by `annotationSortIndex`, to clipboard or file
...
| `templateId` / `includeSubfolders` | `markdown` / `true` | Export defaults                                                                       |
